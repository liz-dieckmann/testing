import { ab as AllowedMimeType, ac as MIME_TYPE_CONFIG } from "./SupportingFilesSection-CeRro-lV.js";
import { b as ENDPOINT_PATTERNS } from "./endpoints-B5IysDN2.js";
import "./expense-form-sections-DQO-bpc2.js";
...
var MemoryCookieStore = class extends Store {
  ...
  findCookie(domain, path, key, callback) {
    ...
    const result = (_b2 = (_a2 = this.idx[domain]) == null ? void 0 : _a2[path]) == null ? void 0 : _b2[key];
  ...
  removeCookie(domain, path, key, callback) {
    var _a2, _b2;
    const promiseCallback = createPromiseCallback(callback);
    (_b2 = (_a2 = this.idx[domain]) == null ? void 0 : _a2[path]) == null ? true : delete _b2[key];
    return promiseCallback.resolve(void 0);
  }
  /**
   * @internal No doc because this is an overload that supports the implementation
    ...
    if (!store.updateCookie) {
      store.updateCookie = async function(_oldCookie, newCookie, cb2) {
        return this.putCookie(newCookie).then(
          ...
          (error2) => cb2 == null ? void 0 : cb2(error2)
        );
      };
    }
...
var CancelableMessageEvent = class extends MessageEvent {
  ...
  preventDefault() {
    if (this.cancelable && !this[kDefaultPrevented$1]) {
...
function getTimestamp$1(options) {
...
function getMessageLength(data) {
  ...
  if (isObject$1(data) && "byteLength" in data) {
    return data.byteLength;
  }
...
function logConnectionOpen(client) {
  ...
  console.groupCollapsed(
    devUtils.formatMessage(`${getTimestamp$1()} %c▶%c ${publicUrl}`),
    `color:${colors.system}`,
    "color:inherit"
  );
...
function logConnectionClose(event) {
  ...
  console.groupCollapsed(
    devUtils.formatMessage(
      `${getTimestamp$1({ milliseconds: true })} %c■%c ${publicUrl}`
    ),
    `color:${colors.system}`,
    "color:inherit"
  );
...
async function logOutgoingClientMessage(event) {
  ...
  console.groupCollapsed(
    devUtils.formatMessage(
      `${getTimestamp$1({ milliseconds: true })} %c${arrow}%c ${publicData} %c${byteLength}%c`
    ),
...
async function logOutgoingMockedClientMessage(event) {
  ...
  console.groupCollapsed(
    devUtils.formatMessage(
      `${getTimestamp$1({ milliseconds: true })} %c⬆%c ${publicData} %c${byteLength}%c`
    ),
    `color:${colors.mocked}`,
    "color:inherit",
    "color:gray;font-weight:normal",
...
function enableMocking(context, options) {
  ...
  context.workerChannel.once("MOCKING_ENABLED", async (event) => {
    ...
    const worker2 = await context.workerPromise;
    printStartMessage({
      quiet: options.quiet,
...
function createResponseListener(context) {
  return (event) => {
    ...
    if ((_a2 = responseMessage.response.type) == null ? void 0 : _a2.includes("opaque")) {
      return;
    }
    const response = responseMessage.response.status === 0 ? Response.error() : new FetchResponse$1(
      /**
       ...
       * for those codes
       */
      FetchResponse$1.isResponseWithBody(responseMessage.response.status) ? responseMessage.response.body : null,
      {
...
var Emitter2 = class {
  ...
  *emitAsGenerator(event) {
  ...
  #callListener(event, listener) {
    ...
    if ((_a2 = listener[kListenerOptions]) == null ? void 0 : _a2.once) {
...
var WorkerChannel = class extends Emitter2 {
  constructor(options) {
    ...
    navigator.serviceWorker.addEventListener("message", async (event) => {
      const worker2 = await this.options.worker;
      if (event.source != null && event.source !== worker2) {
        return;
      }
      if (event.data && isObject$1(event.data) && "type" in event.data) {
...
var SetupWorkerApi = class extends SetupApi {
  ...
  stop() {
    ...
    window.postMessage({ type: "msw/worker:stop" });
    printStopMessage({
      quiet: (_a2 = this.context.startOptions) == null ? void 0 : _a2.quiet
    });
  }
};
...
class HttpHandler extends RequestHandler {
  ...
  async parse(args) {
    ...
    const match2 = this.info.path ? matchRequestUrl(url, this.info.path, (_a2 = args.resolutionContext) == null ? void 0 : _a2.baseUrl) : { matches: false, params: {} };
    return {
      match: match2,
      cookies
    };
  ...
  async log(args) {
    ...
    console.groupCollapsed(
      devUtils.formatMessage(
        `${getTimestamp$1()} ${args.request.method} ${publicUrl} (%c${loggedResponse.status} ${loggedResponse.statusText}%c)`
      ),
      `color:${statusColor}`,
      "color:inherit"
    );
...
const fileHandlers = [
  ...
  http.post(ENDPOINT_PATTERNS.FILES_SUPPORTING_UPLOAD, async ({ request }) => {
    ...
    try {
      ...
      const uploadedFile = {
        ...
        type: fileExtension,
        mimeType: file.type,
        // MIME type already validated and set by client
  ...
  http.post(ENDPOINT_PATTERNS.FILES_RECEIPTS_UPLOAD, async ({ request }) => {
    ...
    try {
      ...
      const fileId = `receipt_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
      const fileExtension = ((_a2 = file.name.split(".").pop()) == null ? void 0 : _a2.toLowerCase()) || "unknown";
      const filename = `${fileId}.${fileExtension}`;
      const cloudStorageUrl = generateCloudStorageUrl(fileId, fileExtension);
      const blobUrl = URL.createObjectURL(file);
      ...
      console.log("✅ MSW: File uploaded successfully", {
        ...
        fileSize: `${Math.round(response.fileSize / 1024)}KB`,
        type: response.type,
...
const createDraftHandlers = (config) => {
  var _a2, _b2, _c2, _d, _e;
  const drafts = /* @__PURE__ */ new Map();
  const submitted = /* @__PURE__ */ new Map();
  if (config.initialDrafts) {
    config.initialDrafts.forEach((draft) => drafts.set(draft.id, draft));
  }
  if (config.initialSubmitted) {
    config.initialSubmitted.forEach((item) => submitted.set(item.id, item));
  }
  const delays = {
    save: ((_a2 = config.delays) == null ? void 0 : _a2.save) ?? 600,
    update: ((_b2 = config.delays) == null ? void 0 : _b2.update) ?? 600,
    get: ((_c2 = config.delays) == null ? void 0 : _c2.get) ?? 300,
    delete: ((_d = config.delays) == null ? void 0 : _d.delete) ?? 500,
    submit: ((_e = config.delays) == null ? void 0 : _e.submit) ?? 1e3
  };
  return [
    http.post(config.patterns.drafts, async ({ request }) => {
      ...
      try {
        ...
        const draftId = `${config.draftIdPrefix}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        const draft = {
          id: draftId,
          status: "draft",
          data: body.data,
    ...
    http.put(config.patterns.draftById, async ({ request, params }) => {
      await delay(delays.update);
      const endpoint = new URL(request.url).pathname;
      if (shouldSimulateError(endpoint)) {
        const mockError = generateMockError();
        return HttpResponse.json(
    ...
    http.get(config.patterns.draftById, async ({ params }) => {
    ...
    http.get(config.patterns.drafts, async () => {
      await delay(delays.get);
      const draftList = Array.from(drafts.values()).sort(
        (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
      );
      return HttpResponse.json(draftList, { status: 200 });
    }),
    http.delete(config.patterns.draftById, async ({ request, params }) => {
    ...
    http.post(config.patterns.submit, async ({ request }) => {
      await delay(delays.submit);
      const endpoint = new URL(request.url).pathname;
      if (shouldSimulateError(endpoint)) {
        const mockError = generateMockError();
        return HttpResponse.json(
      ...
      try {
        const body = await request.json();
        const submittedId = `${config.submittedIdPrefix}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        const submittedItem = {
          id: submittedId,
          status: "submitted",
          data: body.data,
        ...
        submitted.set(submittedId, submittedItem);
        return HttpResponse.json(submittedItem, { status: 201 });
      } catch {
        return HttpResponse.json(
    ...
    http.get(config.patterns.list, async () => {
      await delay(delays.get);
      const submittedList = Array.from(submitted.values()).sort(
        (a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime()
      );
      return HttpResponse.json(submittedList, { status: 200 });
    }),
    http.get(config.patterns.byId, async ({ params }) => {
      await delay(delays.get);
      const itemId = params.id;
      const item = submitted.get(itemId);
      if (!item) {
        return HttpResponse.json(
    ...
    ...config.patterns.byIdUnified ? [
      http.get(config.patterns.byIdUnified, async ({ params }) => {
        await delay(delays.get);
        const itemId = params.id;
        ...
        const submittedItem = submitted.get(itemId);
        if (submittedItem) {
          return HttpResponse.json(submittedItem, { status: 200 });
        }
        return HttpResponse.json(
          { error: "Expense not found", code: "404" },
          { status: 404 }
        );
      })
    ] : []
...
const expenseHandlers = createDraftHandlers({
  ...
  patterns: {
    ...
    submit: ENDPOINT_PATTERNS.EXPENSES_SUBMIT,
    list: ENDPOINT_PATTERNS.EXPENSES_LIST,
    byId: ENDPOINT_PATTERNS.EXPENSE_BY_ID,
    byIdUnified: ENDPOINT_PATTERNS.EXPENSE_BY_ID_UNIFIED
  },
  initialDrafts: mockExpenseDrafts,
  initialSubmitted: mockSubmittedExpenses
});
const mileageHandlers = createDraftHandlers({
  ...
  patterns: {
    ...
    submit: ENDPOINT_PATTERNS.MILEAGE_SUBMIT,
    list: ENDPOINT_PATTERNS.MILEAGE_LIST,
    byId: ENDPOINT_PATTERNS.MILEAGE_BY_ID
  },
  initialDrafts: mockMileageDrafts,
  initialSubmitted: mockSubmittedMileage
});
const mockExpenseTypes = /* @__PURE__ */ new Map();
const initializeMockExpenseTypes = () => {
  mockExpenseTypes.set("htson", [
    {
      id: 1,
      physicalCompanyId: 1,
      logicalCompanyId: 1,
      expenseTypeName: "Office Supplies",
      expenseTypeDescription: "General office supplies and materials",
      formTypeId: 1,
      mileageRateId: null,
    ...
    },
    {
      ...
      expenseTypeDescription: "Business travel expenses",
      formTypeId: 1,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
    ...
    },
    {
      ...
      expenseTypeDescription: "Client meals and entertainment",
      formTypeId: 3,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
    ...
    },
    {
      ...
      expenseTypeDescription: "Business mileage with standard allowance",
      formTypeId: 2,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
    ...
    },
    {
      ...
      expenseTypeDescription: "Personal vehicle business use mileage",
      formTypeId: 2,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
    ...
    },
    {
      ...
      physicalCompanyId: 1,
      logicalCompanyId: 1,
      expenseTypeName: "Mileage Company Vehicle",
      expenseTypeDescription: "Company vehicle business mileage tracking",
      formTypeId: 2,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
  ...
  mockExpenseTypes.set("kore", [
    {
      id: 7,
      physicalCompanyId: 2,
      logicalCompanyId: 2,
      expenseTypeName: "Software & Tools",
      expenseTypeDescription: "Software licenses and development tools",
      formTypeId: 1,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
    ...
    },
    {
      ...
      expenseTypeDescription: "Mileage for client site visits",
      formTypeId: 2,
      mileageRateId: null,
      mileageEffectiveRate: null,
      isActive: true,
...
const configurationHandlers = [
  http.get(ENDPOINT_PATTERNS.CONFIG_EXPENSE_TYPES, async ({ params }) => {
    ...
    const expenseTypes = mockExpenseTypes.get(companyShortName) || [];
  ...
  http.post(ENDPOINT_PATTERNS.CONFIG_EXPENSE_TYPE_CRUD, async ({ request, params }) => {
    ...
    try {
      const companyShortName = params.company;
      const body = await request.json();
      const companyTypes = mockExpenseTypes.get(companyShortName) || [];
      const maxId = companyTypes.length > 0 ? Math.max(...companyTypes.map((t) => t.id)) : 0;
      const newExpenseType = {
        ...
        mileageRateId: body.mileageRate,
        mileageEffectiveRate: null,
        isActive: true,
      ...
      companyTypes.push(newExpenseType);
      mockExpenseTypes.set(companyShortName, companyTypes);
      return HttpResponse.json(newExpenseType, { status: 201 });
    } catch (error2) {
      return HttpResponse.json(
  ...
  http.put(ENDPOINT_PATTERNS.CONFIG_EXPENSE_TYPE_CRUD, async ({ request, params }) => {
    ...
    try {
      ...
      const body = await request.json();
      const companyTypes = mockExpenseTypes.get(companyShortName) || [];
      const expenseTypeIndex = companyTypes.findIndex((t) => t.id === body.id);
      ...
      const existingType = companyTypes[expenseTypeIndex];
      const updatedType = {
        ...existingType,
        expenseTypeName: body.expenseTypeName ?? existingType.expenseTypeName,
        expenseTypeDescription: body.expenseTypeDescription ?? existingType.expenseTypeDescription,
        isActive: body.isActive ?? existingType.isActive,
        updatedDate: (/* @__PURE__ */ new Date()).toISOString(),
        updatedBy: body.updatedBy || "system"
      };
      companyTypes[expenseTypeIndex] = updatedType;
      mockExpenseTypes.set(companyShortName, companyTypes);
      return HttpResponse.json(updatedType, { status: 200 });
    } catch (error2) {
      return HttpResponse.json(
...
const isExpenseMock = (item) => {
  return "vendor" in item.data;
};
const addItemType = (item) => {
  if (isExpenseMock(item)) {
...
const searchById = (items, search) => {
  if (!(search == null ? void 0 : search.trim())) return items;
  return items.filter((item) => item.id === search.trim());
};
...
const sortItems = (items, sortBy, sortOrder) => {
  ...
  return [...items].sort((a, b) => {
    const valueA = getTimestamp(a, sortBy || "updatedAt");
    const valueB = getTimestamp(b, sortBy || "updatedAt");
    if (valueA === valueB) {
...
const expensesListHandlers = [
  http.get(ENDPOINT_PATTERNS.EXPENSES_LIST_UNIFIED, async ({ request }) => {
    await delay(300);
    const url = new URL(request.url);
    const queryParams = {
    ...
    const allMockItems = [
      ...mockExpenseDrafts,
      ...mockSubmittedExpenses,
      ...mockMileageDrafts,
      ...mockSubmittedMileage
    ];
    const itemsWithType = allMockItems.map(addItemType);
    let result = filterByStatus(itemsWithType, queryParams.status);
    result = searchById(result, queryParams.search);
    result = sortItems(result, queryParams.sortBy, queryParams.sortOrder);
    const response = {
      items: result,
      total: result.length,
      params: queryParams
...
class MSWStrategy {
  ...
  async initialize() {
    if (this.active) {
      console.warn("MSW is already active");
      return;
    }
    try {
      ...
      console.error("Failed to start MSW:", error2);
      throw error2;
    }
  }
  async stop() {
