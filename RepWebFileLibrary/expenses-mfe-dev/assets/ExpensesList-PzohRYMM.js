...
import { b as apiClient, p, H as Hs, C as Ca, _ as _r, g as On, L as La, h as Ls, i as Sa, R as Ra, S as Sn } from "./axiosInstance-Wzm5FGtt.js";
...
const ExpensesList = () => {
  ...
  const handleRowClick = useCallback((row) => {
    const { id, expenseType, data } = row.original;
    if (expenseType === "mileage") {
      const formType = "formType" in data ? data.formType : "trip";
      if (formType === "period") {
        navigate(`/expenses/mileage-period/${id}`);
      } else {
        navigate(`/expenses/mileage-trip/${id}`);
      }
    } else {
