...
import { s as string, o as object, d as boolean, e as custom, f as array, g as unknown, n as number, a as useForm, b as a, c as useWatch, h as useFormState, C as Controller } from "./zod-B0omii61.js";
import { importShared } from "./__federation_fn_import-DD1J_cWq.js";
import { j as jsxRuntimeExports } from "./jsx-runtime-CzdF90_e.js";
import { x as Ta, L as La, O as Oa, B as Ba, y as Ga, A as Aa, G as Ge, D as Da, P as Pa, r as Dn, z as za, F as gr, I as sa, J as ua, Q as Qo, v as vr, w as wr, S as Sr, K as os, N as as, R as ss, l as ls, U as ms, V as He, W as ra } from "./createLucideIcon-DxTSH2Gc.js";
import "./api-We2Hav-L.js";
import { L as LoadingSpinner, P as Plus } from "./LoadingSpinner-BxtRdqxJ.js";
...
const { useCallback: useCallback$g, useEffect: useEffect$d, useMemo: useMemo$f, useRef: useRef$a } = await importShared("react");
function useBaseExpenseForm(config2, props) {
  ...
  const handleFormSubmit = useCallback$g(
    async (data) => {
      try {
  ...
  const handleDraftSave = useCallback$g(
    async () => {
      try {
  ...
  const validateForm = useCallback$g(async () => {
    const isFormValid = await trigger();
    return isFormValid && canSubmit;
  }, [trigger, canSubmit]);
  const getSubmitButtonState = useCallback$g(() => {
    if (isSubmitting) {
  ...
  const getSaveDraftButtonState = useCallback$g(() => {
    if (isDrafting) {
...
const useCostAllocation = ({
  ...
  const allocatedPercentage = useMemo$e(() => {
    const totalPercentageDecimal = allocations.reduce(
      (sum2, allocation) => sum2.plus(allocation.percentage),
      new Decimal(0)
    );
...
const useCostAllocationHandlers = (setValue, getValues, costAllocationsField, isEqualSplitField) => {
  const disableEqualSplit = useDisableEqualSplit(setValue, getValues, isEqualSplitField);
  const addAllocation = useCallback$e(
    (type) => {
    ...
    [setValue, getValues, costAllocationsField, disableEqualSplit]
  );
  const updateAllocationEntity = useCallback$e(
    (id, entity) => {
      const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const removeAllocation = useCallback$e(
    (id) => {
      const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const getSelectedValue = useCallback$e(
...
const { useCallback: useCallback$d, useEffect: useEffect$c, useRef: useRef$9 } = await importShared("react");
const useAllocationSync = ({
  ...
  const disableEqualSplit = useDisableEqualSplit(setValue, getValues, isEqualSplitField);
  const setAmountEditing = useCallback$d((editing) => {
    isEditingAmountRef.current = editing;
  }, []);
  const setPercentageEditing = useCallback$d((editing) => {
    isEditingPercentageRef.current = editing;
  }, []);
  const syncPercentageFromAmount = useCallback$d(
    (amount) => {
  ...
  const syncAmountFromPercentage = useCallback$d(
    (percentage) => {
...
const { useCallback: useCallback$c, useEffect: useEffect$b, useMemo: useMemo$c } = await importShared("react");
const useEqualSplit = ({
  allocations,
  isEqualSplit,
  setValue,
  ...
  const distributeEqually = useCallback$c(() => {
    const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const toggleEqualSplit = useCallback$c(() => {
...
const { useCallback: useCallback$b, useMemo: useMemo$b } = await importShared("react");
...
const { useCallback: useCallback$a, useEffect: useEffect$9, useMemo: useMemo$a, useRef: useRef$7 } = await importShared("react");
const { useCallback: useCallback$9, useMemo: useMemo$9 } = await importShared("react");
const useExpenseDetailsHandlers = (setValue) => {
  const handleCurrencyModeChange = useCallback$9(
    (isDifferent) => {
      setValue("isDifferentCurrency", isDifferent, { shouldValidate: true });
    },
    [setValue]
  );
  const handleNetCurrencyChange = useCallback$9(
    (currency) => {
      setValue("netCurrency", currency, { shouldValidate: true });
    },
    [setValue]
  );
  const handleTotalCurrencyChange = useCallback$9(
    (currency) => {
...
const { useCallback: useCallback$8, useImperativeHandle, useRef: useRef$6, useEffect: useEffect$8 } = await importShared("react");
const useFormImperativeHandle = ({
  ...
  const handleFormSubmit = useCallback$8(async () => {
    var _a;
  ...
  const handleDraftSave = useCallback$8(async () => {
...
const useFileHandlers = (options) => {
  ...
  const handleReceiptChange = useCallback$7(
    (attachment) => {
      setValue(
  ...
  const handleSupportingFilesChange = useCallback$7(
    (attachments) => {
...
const { useCallback: useCallback$6, useEffect: useEffect$6, useRef: useRef$4 } = await importShared("react");
function useDebouncedCallback(callback, delay) {
  ...
  const debouncedCallback = useCallback$6(
    (...args) => {
...
const { useCallback: useCallback$5, useRef: useRef$3 } = await importShared("react");
...
const useAutoSave = (options) => {
  ...
  const performAutoSave = useCallback$5(() => {
...
const { useCallback: useCallback$3, useEffect: useEffect$2, useRef: useRef$2, useState: useState$3 } = React$3;
const ReceiptUpload = ({
  onReceiptChange,
  onUploadingChange,
  initialReceipt,
...
const { Fragment, memo: memo$4, useCallback: useCallback$2, useContext, useMemo: useMemo$2, useState: useState$2 } = await importShared("react");
const FormSectionComponent = ({
  ...
  const handleDisabledChange = useCallback$2((disabled) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    ConditionalSection,
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { "data-section-id": section.id, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Da, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Pa, { children: headerActionsNode ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Dn,
            {
              title: section.title,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(za, { children: section.render(finalProps) })
      ] }) }, section.id)
    }
  );
};
const FormSection = memo$4(FormSectionComponent);
...
const { memo: memo$3, useMemo } = await importShared("react");
...
const CostAllocationField = memo$3(
  CostAllocationFieldComponent,
  (prev, next) => {
    var _a, _b, _c, _d;
    return prev.index === next.index && prev.type === next.type && prev.totalAmount === next.totalAmount && prev.currencyCode === next.currencyCode && prev.disabled === next.disabled && ((_a = prev.value) == null ? void 0 : _a.value) === ((_b = next.value) == null ? void 0 : _b.value) && ((_c = prev.value) == null ? void 0 : _c.label) === ((_d = next.value) == null ? void 0 : _d.label) && prev.isEqualSplitField === next.isEqualSplitField;
  }
);
...
const { memo: memo$1, useState: useState$1, useCallback: useCallback$1 } = await importShared("react");
const AddAllocationExpandable = memo$1(({
  onTypeSelect,
  disabled = false
}) => {
  const [isOpen, setIsOpen] = useState$1(false);
  const handleTypeSelect = useCallback$1(
    (type) => {
      onTypeSelect(type);
      setIsOpen(false);
    },
...
function CostAllocationSectionComponent({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(ra, { value: progressValue, disabled: isProgressDisabled, error: progressError }),
...
export {
  mileageJustificationSchema as A,
  BaseExpenseFormRenderer as B,
  ConfirmDialog as C,
  AllowedMimeType as D,
  ExpenseFormField as E,
  FormSectionType as F,
  MIME_TYPE_CONFIG as G,
  MileageTripFormField as M,
  ReceiptUpload as R,
  ...
  FormRenderer as p,
  affidavitExpenseValidationStrategy as q,
  useAutoSave as r,
  expenseDetailsSchema as s,
  expenseJustificationSchema as t,
  useFormFieldValues as u,
  costAllocationSchema as v,
  additionalCommentsSchema as w,
  CostAllocationHeaderActions as x,
  CostAllocationSection as y,
